...
void write_grayScale_bitmap(unsigned char input_image_array[BMP_WIDTH][BMP_HEIGTH], char * output_file_path){
  if (out_bmp == NULL) {
    _throw_error("The function 'read_bitmap' must be called at least once before calling the function 'write_bitmap'.");
  }
  unsigned char val;
  for (int x = 0; x < BMP_WIDTH; x++)
  {
    for (int y = 0; y < BMP_HEIGTH; y++)
    {
      val = input_image_array[x][BMP_HEIGTH-1-y];
      set_pixel_rgb(out_bmp, x, y, val, val, val);
    }
  }
  bwrite(out_bmp, output_file_path);
}

void write_binary_bitmap(unsigned char input_image_array[BMP_WIDTH][BMP_HEIGTH], char * output_file_path){
  if (out_bmp == NULL) {
    _throw_error("The function 'read_bitmap' must be called at least once before calling the function 'write_bitmap'.");
  }
  unsigned char val;
  for (int x = 0; x < BMP_WIDTH; x++)
  {
    for (int y = 0; y < BMP_HEIGTH; y++)
    {
      val = input_image_array[x][BMP_HEIGTH-1-y] ? 255 : 0;
      set_pixel_rgb(out_bmp, x, y, val, val, val);
    }
  }
  bwrite(out_bmp, output_file_path);
